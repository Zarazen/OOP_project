...
private:
	...
	void createMatches();
	void setRound(int& startPos, const int* r1, const int* r2, int& mult);
	void shuffle(int* r1, int* r2);
	void oddRound(int& pos, const int* r1, const int* r2, int& mult);
	void evenRound(int& pos, const int* r1, const int* r2, int& mult);
	char* setDate(const int& addMult);
	...
	Match* end() {
		...
		return &matches[numTeams * (numTeams - 1) / 2];
